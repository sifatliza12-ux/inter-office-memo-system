...
function NotificationBell() {
  ...
  return (
    <div ref={containerRef} className="relative">
      <button
        onClick={openPanel}
        className="relative rounded-md p-2 text-stone-500 transition-colors hover:bg-stone-100 hover:text-plum-700"
      ...
      >
        ...
        {unreadCount > 0 && (
          <span className="absolute right-0.5 top-0.5 flex h-4 min-w-[1rem] items-center justify-center rounded-full bg-terracotta-500 px-1 text-[10px] font-semibold text-white">
      ...
      {open && (
        <div className="absolute right-0 z-10 mt-2 w-80 animate-fade-in-up rounded-xl border border-stone-200 bg-white shadow-card-hover">
          <div className="flex items-center justify-between border-b border-stone-100 px-3.5 py-2.5">
            ...
            <button onClick={handleMarkAllRead} className="text-xs font-medium text-plum-600 hover:text-plum-800 hover:underline">
              Mark all read
            </button>
          </div>
              ...
              notifications.map((notification) => (
                <button
                  ...
                  className={`block w-full border-b border-stone-50 px-3.5 py-2.5 text-left text-sm transition-colors hover:bg-stone-50 ${
                    notification.isRead ? '' : 'bg-plum-50/60'
                  }`}
                >
